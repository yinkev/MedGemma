...
APP_NAME=${APP_NAME:-MedASRBar}
BUNDLE_ID=${BUNDLE_ID:-com.medasr.bar}
...
<dict>
    ...
    <key>NSMicrophoneUsageDescription</key><string>MedASR uses the microphone to transcribe speech locally.</string>
    <key>NSScreenCaptureUsageDescription</key><string>MedASR can capture system audio for transcription.</string>
...
AUDIOCAPTURE_SRC="$REPO_ROOT/native/audiocapture"
if [[ -x "$AUDIOCAPTURE_SRC" ]]; then
  cp "$AUDIOCAPTURE_SRC" "$APP/Contents/MacOS/audiocapture"
  chmod +x "$APP/Contents/MacOS/audiocapture"
fi
